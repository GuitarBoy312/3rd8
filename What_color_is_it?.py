...
3. 그 후, 너는 "What color is it?" 이라고 질문해. 질문할 때 마지막에 💛💚💙🖤🤍💔 이 6개의 이모지 중 하나를 질문 끝에 무작위로 매번 바꿔가며 붙여. 6개의 이모지를 무작위로 한번씩 선택한 후에 다시 무작위로 한번씩 선택하고 반복해.
그러면 내가 이모지의 색을 보고 대답할거야. 그리고 질문할 때마다 이모지를 바꿔줘.
